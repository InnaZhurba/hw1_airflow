...
with DAG(dag_id="dag_weather_v5", schedule_interval=None, start_date=datetime(2023, 11, 22), catchup=False) as dag:
    ...
    for city in cities:
        with TaskGroup(f"weather_tasks_{city}") as weather_tasks:
            ...
            # get_weather_api_url = PythonOperator(
            #     task_id=f"get_weather_api_url_{city}",
            #     python_callable=_get_weather_api_url,
            #     op_args=[city]
            # )

            # fetch_weather_data = SimpleHttpOperator(
            #     task_id=f"fetch_weather_data_{city}",
            #     http_conn_id="weather_conn",
            #     method="GET",
            #     endpoint=f"{{{{ task_instance.xcom_pull(task_ids='get_weather_api_url_{city}') }}}}",
            #     response_filter=lambda x: json.loads(x.text),
            #     log_response=True 
            # )

            # inject_data = SqliteOperator(
            #     task_id=f"inject_data_{city}", sqlite_conn_id="airflow_conn", 
            #     sql=f"""
